-- Ethan Coyle
-- Dr. LopaMudra
-- Assignment 5 SQL and at the bottom is Assignmnet 6 query for Customer 1
...
-- ASSIGNMENT 6 SQL JOIN
-- For Customer number 1, display customer ID, customer name, each order number, order date, and the total
-- quantity of all product items per order. 
-- here we are selecting the customers name, the orderid of the customer  and order date wtih the aggregation groubed 
-- by the sum  of their ordered quantity and is grouped by the the orderid for customer 1  running the below will display 
-- the aggregation found for customer 1  the output collumns for out expected output will be :
-- | customerID | customerName     | OrderID | OrderDate  | TotalQuantity |
-- below displays the speified query to satisfy this output 
SELECT  CustomerName, Order_T.OrderID, Customer_T.CustomerID,OrderDate, SUM(OrderedQuantity) AS TotalQuantity FROM Customer_T , Order_T, OrderLine_T WHERE Order_T.CustomerID = Customer_T.CustomerID and OrderLine_T.OrderID = Order_T.OrderID AND Customer_T.CustomerID = 1 GROUP BY OrderID;
